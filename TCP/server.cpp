...
//set args -i 134.162.33.21:1090 -hlsv /sites/virex/content/video/videohls3 -fps 25 -parts 6 -sdur 5
...
void *InWorker(void *Parm){
    ...
    if(len < 1){
       printf("input stream closed or failed 1\n");
       return 0;
    }
    //if(Writer != NULL){
    //   Writer->Start();
    ...
    for(;;){
        ...
        if(len < 1){
           printf("input stream closed or failed 2\n");
           return 0;
        }
        ...
        if(len < 1 || len != Dw[0]){
           printf("input stream closed or failed 3, ret %d, size: %d\n", len, Dw[0]);
           return 0;
        }
...
void PrintHelp(){
    printf("Use: mc_server -[i|f] input -[wbs|hls|clnt] output\n"
           ...
           "-hls {directory} generate the hls-stream files from the input and save them to a directory\n"
           "-hlsv {directory} generate the live hls-stream files from the input and save them to a directory\n"
